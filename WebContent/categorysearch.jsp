...
<%! 
	// request.getParameter("mainclass")
//  request.getParameter("detailclass")
// lomboz 存在一个很严重的问题  当我用html注释在第一行时 下面的全部出错 导致整个程序接近崩溃 感觉排错有点问题
...
// 程序的不足之处 日期怎么可能手工大的  肯定按一下日期会有一个阅历跑出来然你选择
	...
	strpageno = request.getParameter("pageno");
		...
		//还有这个方法 怪不得我想不到  拿到字符串数组
		 categoryarray = request.getParameterValues("detailclass");
		categoryid = Integer.parseInt(request.getParameter("detailclass"));
				 idArray = new int[categoryarray.length];
				for(int i = 0; i < idArray.length ; i ++){
					postcategoryId += "&categoryId="+categoryarray[i];
					idArray[i] = Integer.parseInt(categoryarray[i]);
					//System.out.println(idArray[i]);
				}
		
		//默认的全部分类 进行判断
		//System.out.println("sd");
				totalpage = ProductManager.getInstance().getTotalpage(pl,idArray, "", snormalPrice,enormalPrice, smemberPrice,
...
<center>搜索结果</center>
	...
	<!-- 接下来是 跳转页面的问题  因为跳转后 程序s头还有getparameter的东西 如果不在后面跟上参数 拿不到 空指针错误
	 ...
	 -->
		共 <%= totalpage %> 页
		第 <%= pageno %> 页<p/>
		<a href="categorysearch.jsp?pageno=1&action=<%= action %>
		&detailclass=<%= categoryid  %>&snormalPrice=<%= snormalPrice%>&enormalPrice=<%= enormalPrice%>
		&smemberPrice=<%= smemberPrice%>&ememberPrice=<%= ememberPrice%>">首页</a>
		
		<a href="categorysearch.jsp?pageno=<%= (pageno-1) %>&action=<%= action %>
		&detailclass=<%= categoryid  %>&snormalPrice=<%= snormalPrice%>&enormalPrice=<%= enormalPrice%>
		...
		<a href="categorysearch.jsp?pageno=<%= totalpage %>&action=<%= action %>
		&detailclass=<%= categoryid  %>&snormalPrice=<%= snormalPrice%>&enormalPrice=<%= enormalPrice%>
		&smemberPrice=<%= smemberPrice%>&ememberPrice=<%= ememberPrice%>">最后一页</a>
	<br><a href="index2.jsp">返回主页</a>
	</center>
<%		 }
		  else{
			
%>
抱歉，此类别下没有任何宝贝，请返回重新选择分类。
<br><a href="index2.jsp">返回主页</a>
<% 
		}
