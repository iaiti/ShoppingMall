...
// 程序的不足之处 日期怎么可能手工大的  肯定按一下日期会有一个阅历跑出来然你选择
	...
	strpageno = request.getParameter("pageno");
		//当用户 不输入任何东西时 将所有的变量设成 特殊值 因为getproductlist方法里面需要传参
		 keyword = new String(request.getParameter("search").getBytes("ISO-8859-1"),"UTF-8");
		//还有这个方法 怪不得我想不到  拿到字符串数组
		//默认的全部分类 进行判断
		//System.out.println("sd");
			if(keyword==null||keyword==""){
				totalpage = ProductManager.getInstance().getTotalpageWithoutId(pl, "", snormalPrice,enormalPrice, smemberPrice,
						ememberPrice, sp, ep,pageno,pagesize);
			}else{
				//判断关键词是否为空 不像价格那样赋值了-1 如果两者都没有 记录是拿不到的
				totalpage = ProductManager.getInstance().getTotalpageWithoutId(pl, keyword, snormalPrice,enormalPrice, smemberPrice,
						ememberPrice, sp, ep,pageno,pagesize);
			}
		if(strpageno!=null){
...
<center>搜索结果</center>
	<table border=1 align=center>
		...
		<tr>
			...
			<td><a href=productdetail.jsp?id=<%= pd.getId() %> >查看宝贝详情</a></td>
		</tr>
		<%
				}
		%>
	...
	<!-- 接下来是 跳转页面的问题  因为跳转后 程序s头还有getparameter的东西 如果不在后面跟上参数 拿不到 空指针错误
	 ...
	 -->
		共 <%= totalpage %> 页
		第 <%= pageno %> 页<p/>
		<a href="simplesearchwhole.jsp?pageno=1&action=<%= action %>
		&search=<%= keyword%>&categoryId=<%= categoryId %>&snormalPrice=<%= snormalPrice%>&enormalPrice=<%= enormalPrice%>
		&smemberPrice=<%= smemberPrice%>&ememberPrice=<%= ememberPrice%>">首页</a>
		
		<a href="simplesearchwhole.jsp?pageno=<%= (pageno-1) %>&action=<%= action %>
		&search=<%= keyword%>&categoryId=<%= categoryId %>&snormalPrice=<%= snormalPrice%>&enormalPrice=<%= enormalPrice%>
		...
		<a href="simplesearchwhole.jsp?pageno=<%= totalpage %>&action=<%= action %>
		&search=<%= keyword%>&categoryId=<%= categoryId %>&snormalPrice=<%= snormalPrice%>&enormalPrice=<%= enormalPrice%>
